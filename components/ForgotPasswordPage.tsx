...
const ForgotPasswordPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log("[PW_RESET_CLIENT] response", res.status, json);

            if (res.status === 200 && json.success === true) {
                setStatus("success");
            } else if (res.status === 404 && json.code === "USER_NOT_FOUND") {
                setStatus("error");
                setErrorMessage("No account found for this email. Please check the address or sign up.");
            } else {
                setStatus("error");
                setErrorMessage(json.message || "Something went wrong while sending the reset email. Please try again later.");
            }
        } catch (err: any) {
