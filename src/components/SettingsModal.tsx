      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-2xl h-[600px] flex overflow-hidden shadow-2xl animate-fade-in transition-colors duration-200">
        ...
        <div className="w-1/3 bg-slate-50 dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 p-4 flex flex-col gap-2 transition-colors duration-200">
          ...
          <button
            ...
            className={`text-left px-3 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-3 ${activeTab === 'general' ? 'bg-medical-100 dark:bg-medical-900/30 text-medical-700 dark:text-medical-400' : 'text-slate-600 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-700'}`}
          >
          ...
          <button
            ...
            className={`text-left px-3 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-3 ${activeTab === 'account' ? 'bg-medical-100 dark:bg-medical-900/30 text-medical-700 dark:text-medical-400' : 'text-slate-600 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-700'}`}
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        <div className="flex-1 p-8 overflow-y-auto relative">
          <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:text-slate-200 dark:hover:text-white">
              ...
              <div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between pb-6 border-b border-slate-100 dark:border-slate-800">
                    <div>
                      <div className="text-sm font-medium text-slate-700 dark:text-white">Interface Theme</div>
                      <div className="text-xs text-slate-500 dark:text-slate-300 mt-1">Select your preferred color scheme</div>
                    </div>
                    <select
                      value={theme}
                  ...
                  <div className="flex items-center justify-between pb-6 border-b border-slate-100 dark:border-slate-800">
                    <div>
                      <div className="text-sm font-medium text-slate-700 dark:text-white">Response Language</div>
                      <div className="text-xs text-slate-500 dark:text-slate-300 mt-1">Language for AI answers and dictionary</div>
                    </div>
                    <select
                      value={language}
                  ...
                  <div className="pb-6 border-b border-slate-100 dark:border-slate-800">
                    <div className="mb-4">
                      <div className="text-sm font-medium text-slate-700 dark:text-white">Default Clinical Location</div>
                      <div className="text-xs text-slate-500 dark:text-slate-300 mt-1">Set the primary medical jurisdiction for guidelines</div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 dark:text-slate-200 mb-1.5">Region</label>
                        <select
                          value={region}
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 dark:text-slate-200 mb-1.5">Country</label>
                        <select
                          value={country}
                  ...
                  <div className="flex items-center justify-between pb-6 border-b border-slate-100 dark:border-slate-800">
                    <div>
                      <div className="text-sm font-medium text-slate-700 dark:text-white">Font Size</div>
                      <div className="text-xs text-slate-500 dark:text-slate-300 mt-1">Adjust text size for reading comfort</div>
                    </div>
                    <div className="flex bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
                      <button
                        onClick={() => setFontSize('small')}
                        className={`px-3 py-1 text-xs font-medium rounded transition-all ${fontSize === 'small' ? 'bg-white dark:bg-slate-600 shadow-sm text-slate-800 dark:text-white ring-1 ring-slate-200 dark:ring-slate-500' : 'text-slate-500 dark:text-white hover:text-slate-700 dark:hover:text-slate-200'}`}
                      >
                        Aa
                      </button>
                      <button
                        onClick={() => setFontSize('normal')}
                        className={`px-3 py-1 text-sm font-medium rounded transition-all ${fontSize === 'normal' ? 'bg-white dark:bg-slate-600 shadow-sm text-slate-800 dark:text-white ring-1 ring-slate-200 dark:ring-slate-500' : 'text-slate-500 dark:text-white hover:text-slate-700 dark:hover:text-slate-200'}`}
                      >
                        Aa
                      </button>
                      <button
                        onClick={() => setFontSize('large')}
                        className={`px-3 py-1 text-base font-medium rounded transition-all ${fontSize === 'large' ? 'bg-white dark:bg-slate-600 shadow-sm text-slate-800 dark:text-white ring-1 ring-slate-200 dark:ring-slate-500' : 'text-slate-500 dark:text-white hover:text-slate-700 dark:hover:text-slate-200'}`}
                      >
                        Aa
                      </button>
                    </div>
          ...
          {activeTab === 'account' && (
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-5 mb-8 pb-8 border-b border-slate-100 dark:border-slate-800">
                ...
                <div>
                  ...
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${isAuthenticated ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                    {isAuthenticated ? (userRole || 'Professional') : 'Guest'}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-5">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 dark:text-slate-200 uppercase tracking-wider mb-2">Full Name</label>
                  <input
                    type="text"
                    value={userProfile?.full_name || ''}
                    disabled
                    className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2.5 text-sm text-slate-500 dark:text-indigo-200 cursor-not-allowed"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2.5 text-sm text-slate-500 dark:text-indigo-200 cursor-not-allowed"
                  />
                  <p className="mt-1.5 text-[11px] text-slate-400 dark:text-slate-300">Contact your administrator to change your email.</p>
                </div>

                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-xs font-semibold text-slate-500 dark:text-slate-200 uppercase tracking-wider">Professional Role</label>
                    <span className="text-[10px] text-medical-600 dark:text-medical-400 bg-medical-50 dark:bg-medical-900/20 px-2 py-0.5 rounded-full font-medium">
                      Adapts AI Answers
                    </span>
                  </div>
                  ...
                  <div className="space-y-3">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (val === 'Other') {
                          setUserRole(''); // Clear for custom input
                        } else {
                          setUserRole(val);
                        }
                      }}
                      className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2.5 text-sm text-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-medical-500 appearance-none cursor-pointer"
                    >
                    ...
                    {/* Show Custom Input if 'Other' is selected (implied by userRole not being in standard list) */}
                    {(!PROFESSIONAL_ROLES.includes(userRole) || userRole === 'Other' || userRole === '') && (
                      <input
                        type="text"
                        placeholder="Specify your role (e.g. Surgeon, Dentist)..."
                        value={userRole === 'Other' ? '' : userRole} // handle 'Other' literal if explicitly set
                        onChange={(e) => setUserRole(e.target.value)}
                        className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2.5 text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-medical-500 animate-fade-in"
                        autoFocus
                      />
                    )}
                  </div>
                  ...
                  <div className="border-t border-slate-100 dark:border-slate-800 pt-8 mt-4">
                    ...
                    <div className="space-y-3">
                      <button
                        ...
                        className="w-full text-left px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors flex items-center justify-between group"
                      >
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span>{subscription ? 'Manage Subscription' : 'Upgrade Plan'}</span>
                            {subscription ? (
                              <span className="text-xs text-slate-500 dark:text-slate-300 font-normal">
